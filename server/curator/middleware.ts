 ...
 */
const isCurrentSessionCuratorExists = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  if (req.session.curatorId) {
    const curator = await CuratorCollection.findOneByCuratorId(
...
const isAccountExists = async (
  ...
  if (user) {
    ...
    res
      .status(401)
      .json({ error: "Invalid curator login credentials provided." });
  }
};
