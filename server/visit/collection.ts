...
class VisitCollection {
   ...
   */
  static async startVisit(curatorId: Types.ObjectId | string): Promise<HydratedDocument<Visit>> {
    const date = new Date();
   ...
   */
  static async endVisit(visitId: Types.ObjectId | string): Promise<HydratedDocument<Visit>> {
   ...
   */
  static async addWork(visitId: Types.ObjectId | string, workId: Types.ObjectId): Promise<HydratedDocument<Visit>> {
    const visit = await VisitModel.findOne({_id: visitId});
   ...
   */
  static async findVisit(visitId: Types.ObjectId | string): Promise<HydratedDocument<Visit>> {
   ...
   * @returns 
   */
  static async findAllVisitsByCurator(username: string): Promise<Array<HydratedDocument<Visit>>> {
   ...
   * @param curatorId 
   */
  static async deleteAllVisitsByCurator(curatorId: Types.ObjectId | string): Promise<void> {
